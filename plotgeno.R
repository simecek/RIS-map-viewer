...
plotgeno <- function(tmp) {
  ...
  tmp <- tmp[order(tmp$chr, tmp$from),]
    ...
    if (nrow(tmp)>0) {
      ...
      sel.pal <- names(pal2) %in% tmp$type
      sel.pal.explained <- rep("", sum(sel.pal))
      for (i in 1:length(sel.pal.explained))
        sel.pal.explained[i] <- subset(tmp, type == names(pal2)[sel.pal][i])$type.explained[1]
      ...
      for (i in 1:20) {
        ...
        for (j in which(tmp$chr==chrs[i])) {
          col <- switch(tmp$type[j], "AA" = pal[1], "BB" = pal[2], "AB" = pal[4], "CC"=pal[3], "loss" = "black", gain="black")
  ...
  if (nrow(tmp)+nrow(tmp2)>0) {
    ...
    for (i in 1:20) {
      ...
      for (j in which(tmp2$chr==chrs[i])) {
        col <- switch(tmp2$type[j], "AA" = pal[1], "BB" = pal[2], "AB" = pal[4], "CC"=pal[3], "loss" = "black", gain="black")
